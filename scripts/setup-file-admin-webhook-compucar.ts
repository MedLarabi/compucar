...
async function setupFileAdminWebhook() {
  try {
    // Use the same token approach that worked for Super Admin Bot
    ...
    // Set the webhook - same approach that worked for Super Admin Bot
    ...
    if (result.ok) {
      ...
      // Get webhook info to confirm - same as Super Admin Bot
